...
func TestProcessInteractiveEvents_StripsAgentFooterWhenEnabled(t *testing.T) {
	...
	// Non-silent turns also get a standalone turn-summary message ("✅ Done
	// · ...") after the reply body, so two sends are expected: the filtered
	// answer, then the summary.
	sent := p.getSent()
	if len(sent) != 2 {
		t.Fatalf("sent = %#v, want reply + turn-summary", sent)
	}
	if sent[0] != "answer" {
...
func TestProcessInteractiveEvents_KeepsAgentFooterByDefault(t *testing.T) {
	...
	// Non-silent turns also get a standalone turn-summary message ("✅ Done
	// · ...") after the reply body, so two sends are expected: the
	// unfiltered body, then the summary.
	sent := p.getSent()
	if len(sent) != 2 {
		t.Fatalf("sent = %#v, want reply + turn-summary", sent)
	}
